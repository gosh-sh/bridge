...
├── contracts/an/           ← TVM (gosh-solidity): exchange/ holds eccUSDCBridge, DepositVoucher, EthBeaconLightClient + EthKeccak sources and the compiled artefacts acki-nacki pins by commit for its zerostate (built with SOLD, sold 0.82.0); the top-level EthBeaconLightClient.sol / EthKeccak.sol are the standalone light-client variant
├── crates/
