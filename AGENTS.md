...
├── crates/
│   ├── acki-nacki-interface/  ← Rust traits + mock for AN node communication (REST `BkSetClient` / `BkSetTracker` retired 2026-07-30; BK-set rotation is now proven at the source seam via `bridge_prover_lib::live_driver::LiveProverDriver` — port 8600 REST is internal-only on public shellnet since AN v0.16.3)
...
├── crates/deposit-relayer-daemon/      ← EVM→AN deposit relayer (mirror of bridge-relayer-daemon): listen for `Deposit` events (EthLogSource over alloy) → generate the AN-consumable Halo2 proof triple (SubprocessProofGenerator over deposit-prover) → submit to `TokenBridge.finalizeDeposit` (AnSubmitter). `finalizeDeposit` write gated on the upstream `IAckiNacki`/tvm-sdk client. `deposit-relayer` CLI: watch / prove-one / an-preflight / daemon  (REST `BkSetClient` retired 2026-07-30; BK-set rotation is now proven at the source seam via `bridge_prover_lib::live_driver::LiveProverDriver`)
...
- `acki-nacki-interface`: Async traits (`IAckiNacki`, `TransactionSender`) + mock implementations. **REST `BkSetClient` / `BkSetTracker` retired 2026-07-30**: the AN `/v2/bk_set{,_update}` endpoints (port 8600) are internal-only on public shellnet since AN v0.16.3, and BK-set rotation is now proven at the source seam via `bridge_prover_lib::live_driver::LiveProverDriver` — no separate REST poller needed.
...
# (`sentry-watch` subcommand + `SentryGuardedRelayer` retired 2026-07-30 with the REST BkSetClient/BkSetTracker;
#  BK-set rotation is now proven at the source seam via `bridge_prover_lib::live_driver::LiveProverDriver`.)
...
cd crates/bridge-relayer-daemon && cargo run --bin relayer -- smoke-fixture \
    --fixtures-dir ./fixtures --rpc-url ... --bridge-address ...                         # smoke run (SentryGuardedRelayer/--an-node-url retired 2026-07-30)
...
# (`live_bk_set_sentry` integration test retired 2026-07-30 with the REST BkSetClient/BkSetTracker.)
...
| Crate | Count | Notes |
|------|------|------|
| `bridge-relayer-daemon` | 29 | state persistence (2), `BlockSource` (2), `MockBridgeClient` (4), `Relayer` loop end-to-end (5), **daemon** exponential-backoff + shutdown-aware sleep + `RelayerMetrics` atomic counters + `BackoffConfig::bump` cap (5); BkSetSentry / SentryGuardedRelayer + `live_sentry_*` retired 2026-07-30 with the REST BkSetClient/BkSetTracker |
