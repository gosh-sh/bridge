...
- `docs/zk_halo2_an_side_design.md` — design memo for the AN-side `ZKHALO2VERIFYWITHVK` opcode: gap analysis vs. partner's `ZKHALO2VERIFY` (hard-coded DarkDex VK), proposed stack ABI / gas model / per-VK cache, six-phase roadmap. Companion skeleton lives in `tvm-sdk` branch `serhii/verhalo2shplonk-skeleton`. **Wire-format Q-WIRE-1..5 + Q-NAME-1 FROZEN 2026-05-22** (table in §4): Blake2b transcript, shared `ParamsKZG<Bn256>` per `k`, strict 32-byte LE Fr public inputs, self-describing `Halo2TvmBundle` (magic `b"H2TVMBND"`, format_version `0x01`), commit-SHA-pinned halo2-lib fork + CI gate, opcode at `0xC7 0x4A`. Created 2026-05-17, frozen 2026-05-22.
