...
> **⚠️ Recipient constraint (learned 2026-07-01, real E2E).** The deployed `USDCBridge._parsePublicInputs` reads `anWorkchain = int8(uint8(fr[6]))` and `anAccount = fr[7]`, but the **deposit circuit splits the 32-byte `anAccount` into `anAccountHigh (fr[6])` / `anAccountLow (fr[7])`**. A full 256-bit AN recipient (e.g. a real msig `0x20c2db9c…`) puts a large value in `fr[6]`, so `uint8(fr[6])` **overflows → compute `exit_code 4`** (TVM integer overflow — *not* `ERR_INVALID_ZKPROOF`=220; the `ZKHALO2VERIFYWITHVK` opcode actually *accepts* the proof). The deposit `anAccount` **must have its high 16 bytes = 0** (≤128-bit id, exactly as `docs/user/USER_GUIDE.md` shows `0x00…00ab` and the passing fixtures use `0x…deadbe00`). The credited `0:anAccountLow` is an `Uninit` account (`dapp_id == account_id`) — fine for the deposit-credit leg, but the AN→ETH withdraw-back can't originate from it. Debug ground truth: `depositId=2` had `fr[6]=0x20c2db9c…` → `exit_code 4`; `depositId=3` had `fr[6]=0` → success. (The blocker table below is historical — the VK gap it describes is closed.) **⚠️ Already fixed upstream, pending shellnet redeploy:** `acki-nacki` commit `eab02057e` ("update exchange contract (BRIDGE)", #2271) rewrites `_parsePublicInputs` to `anAccount = (fr[6] << 128) | fr[7]` + `makeAddrStd(0, anAccount)` (drops the `int8` workchain field), so full 256-bit recipients work once shellnet redeploys the new `USDCBridge` (same VkBlob `20cf9018`). **⚠️ #2271 fixed the `.sol` source but did NOT regenerate the committed compiled `.tvc`** — `contracts/0.79.3_compiled/exchange/USDCBridge.tvc` still has code-hash `818fb76d…` = the old-parsing bytecode that is live on shellnet (verified 2026-07-01: deployed-source commit `385886071` and upstream `poseidon_dex` HEAD both compile-artifact-identical to the deployed hash). So the fix requires a **recompile of the #2271 source → new `.tvc` → redeploy**; redeploying the current committed artifact would keep the bug. The high-16-bytes-zero rule only applies to the **currently deployed** pre-#2271 contract — once redeployed, a controllable 256-bit msig recipient becomes usable, unblocking a true full-cycle round-trip.
