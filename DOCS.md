...
| [`AGENTS.md`](AGENTS.md) | Context for agents and contributors: changelog policy, workspaces, build and CI. Rewritten 2026-09-23 against the tree; its paths are current. |
| [`crates/bridge-circuits/docs/BLOCK_ID_ALG_NEW.md`](crates/bridge-circuits/docs/BLOCK_ID_ALG_NEW.md) · [`GLOBAL_HISTORY_DATA_SPEC.md`](crates/bridge-circuits/docs/GLOBAL_HISTORY_DATA_SPEC.md) · [`BRIDGE_PROVER_THINNING_SPEC.md`](crates/bridge-circuits/docs/BRIDGE_PROVER_THINNING_SPEC.md) · [`test_boc.md`](crates/bridge-circuits/docs/test_boc.md) · [`bridge-event-prove-circuit/src/EVENT_LAYOUT_COMPARISON.md`](crates/bridge-circuits/bridge-event-prove-circuit/src/EVENT_LAYOUT_COMPARISON.md) · [`attestation-bls-checker-circuit/PROVER_OPS.md`](crates/bridge-circuits/attestation-bls-checker-circuit/PROVER_OPS.md) · [`PARALLEL_BENCHMARK_N14_REPORT.md`](crates/bridge-circuits/attestation-bls-checker-circuit/PARALLEL_BENCHMARK_N14_REPORT.md) · [`src/bin/PARALLEL_BENCHMARK_RESULTS.md`](crates/bridge-circuits/attestation-bls-checker-circuit/src/bin/PARALLEL_BENCHMARK_RESULTS.md) | **Vendored** with the `crates/bridge-circuits/` sub-workspace (see the `Changed` entry under `## [Unreleased]` in `CHANGELOG.md` that landed the vendoring). Design + reference material carried over from `gosh-sh/acki-nacki-to-eth-bridge-halo2-circuits`. Only two of these — `BLOCK_ID_ALG_NEW.md` and `GLOBAL_HISTORY_DATA_SPEC.md` — pin an upstream commit in-body (`acki-nacki@poseidon_profile_new` HEAD `36cd98721`); the other six pin only files, or nothing. **Not independently re-verified against this repo** — treat as source material and cross-check against the code before acting, not as canon. The two `PARALLEL_BENCHMARK_*` files are point-in-time benchmark snapshots that would be barred by rule 4 (No orphan status reports) if written today; they remain as measured data referenced by `AGENTS.md` and the crate `README.md`, and should be replaced by dated tables inside a verification-genre document during the rewrite. |
