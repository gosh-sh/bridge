...
- **ETH-6 artefacts:** pin `contracts/ethereum/verifiers/SHA256SUMS` and CREATE `extcodehash` in `ShplonkDeployLib` (`PRIMARY_YUL_CODEHASH` / …). Default pairing `ShplonkArtefactPairing.t.sol` — C4 PASS. 1A/1B/C2 quarantined in `ShplonkArtefactPairingPendingN14` until n14 regen. See `audit/findings/BRIDGE-ETH-06/`.
