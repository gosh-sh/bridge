...
**Status:** **closed (documented + tests + remaining-appends view)** — occupancy saturates; SLA uses `anchorRemainingAppends`  
...
| `layerWindowLen(layer)` | Occupancy `0..=128`. Saturates; not SLA headroom (ETH-18). |
| `anchorRemainingAppends(layer, anchor)` | Appends until eviction. 0 = not in window. Alert when ≤ 28. |
...
- SLA + alert recipe in `docs/audit/eth-qc-hardening-runbook.md` § WD-Q1 (`anchorRemainingAppends`).
