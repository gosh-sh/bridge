...
- [x] Reentrancy surface — единый guard `_reentrancyStatus` на всех mutating entrypoints (`deposit`, `verifyBlock`, `applyBkSetUpdate`, `withdrawByProof`, `supplyToAave`, `withdrawFromAave`, `emergencyWithdrawAll`, `harvestYield`). Незащищённые owner-only сеттеры (`setAaveEnabled`, `setLiquidReserveBps`, `setYieldRecipient`, `transferOwnership`) — без внешних вызовов. Mainnet USDC (FiatTokenProxy) не имеет transfer-хуков. Cross-function reentrancy закрыта общим guard. **OK**
- [x] Pause interaction — **#20 / TD-58:** bridge `pause()` / `whenNotPaused` **удалены**; `deposit` только `nonReentrant`. USDC token pause/blacklist — внешний риск (TD-24/58). Покрыто `DepositPauseAsymmetry.t.sol`, `DepositEdgeCases::test_deposit_noPauseGate_alwaysCallable`
...
| Шаг | Что | Строки |
|---|---|---|
| Checks | `amount != 0`, `amount <= MAX_DEPOSIT_AMOUNT`, `anAccount != 0`; `nonReentrant` only (#20) | 579–584 |
...
| A1-F6 | Info | OK | Централизация: owner `transferOwnership` без 2-step; **нет** bridge `pause()` (#20). USDC Circle blacklist/pause на адрес бриджа = внешний ops risk (TD-24). Owner не крадёт principal (SWC-105) | n/a |
...
**Проверка тестового покрытия:** DEP-1..3 — `FuzzVerifiers.t.sol`; pause asymmetry — `DepositPauseAsymmetry.t.sol`, `DepositEdgeCases::test_deposit_noPauseGate_alwaysCallable` (#20); AAVE — `AckiNackiBridgeAave.t.sol`. `AckiNackiBridgePause.t.sol` **удалён** с #20.
...
| PS-1 | **OBSOLETE (#20):** bridge `pause()` removed — user entrypoints not `BridgePaused`-gated; USDC token pause is external (TD-58) |
