...
|----|--------------|----------------------------------------|----------------------------------------|
| QC-OFF-01 | Daemon uses **strict sequential** `depositId` (`next_target = last_processed + 1`). One deposit that never finalizes (persistent `AnRejected`, bad config) **blocks all later depositIds**. | Intentional? Need skip / out-of-order finalize / alert on `attempts_since_progress`? | **closed** — keep HOL; reject out-of-order ([issue #34](https://github.com/gosh-sh/bridge/issues/34)). `--skip-after-attempts` parks a stuck id and advances the cursor (`TickOutcome::Skipped` → `parked_deposit_ids`); production systemd `SKIP_AFTER_ATTEMPTS=64`, CLI default 0. `--max-attempts-warn` on `attempts_since_progress`. Policy: `docs/audit/qc-off-01-hol-policy.md`. |
