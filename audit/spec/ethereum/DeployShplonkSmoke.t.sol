...
/// @notice Phase E / E-06 — production deploy lib wires SHPLONK aggregators, not Groth16 stubs.
/// @dev WD-Q3 complement: mirrors `DeployRealBridge` / `ShplonkDeployLib` stack.
contract DeployShplonkSmokeTest is Test {
    string internal constant ETH_ROOT = "../../../contracts/ethereum/";
    ...
    /// @dev E-06: deploy stack with stub Yul still yields aggregator adapter types.
    function test_E06_shplonkDeployLib_stubYul_wiresAggregatorNotGroth16() public {
        ...
        assertTrue(
            keccak256(type(BridgeWithdrawalAggregatorVerifier).creationCode)
                != keccak256(type(BridgeWithdrawalVerifier).creationCode),
            "production adapter bytecode differs from Groth16 stub"
        );
    }
