...
/// @notice Phase C / A1 — QC-A1-1: per-tx cap is `MAX_DEPOSIT_AMOUNT` (uint64.max on main); aggregate inflow unbounded.
...
contract DepositWhaleCapTest is Test {
    ...
    function test_manyDeposits_aggregateUnbounded() public {
        uint256 n = 20;
        uint256 expectedTotal = n * maxPerTx;

        for (uint256 i = 0; i < n; i++) {
            address user = address(uint160(0x1000 + i));
            UsdcTestLib.depositUsdc(vm, usdc, bridge, user, maxPerTx);
        }
