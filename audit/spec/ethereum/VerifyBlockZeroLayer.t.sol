...
/// @notice Phase C / A2 — QC-A2-3: zero hash in active layer slot is accepted but not windowed.
contract VerifyBlockZeroLayerTest is Test {
    ...
    function test_verifyBlock_activeZeroLayerHash_skipsWindowAppend() public {
        ...
        assertEq(bridge.storedLayerHashes(0), 0, "QC-A2-3: zero stored in active slot");
        assertFalse(bridge.isKnownLayerAnchor(1, 0), "zero not appended to L1 window");
        assertTrue(bridge.isKnownLayerAnchor(2, 0xA2), "non-zero L2 windowed");
        assertTrue(bridge.isKnownLayerAnchor(3, 0xA3), "non-zero L3 windowed");
    }
}
