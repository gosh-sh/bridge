...
func runSetup(proofFile string) {
	fmt.Println("=== BK Set Rotation Groth16 Setup ===")
	...
	circuit, err := NewBkSetRotationCircuit(proofData)
	if err != nil {
...
func runProve(proofFile string) {
	fmt.Println("=== BK Set Rotation Groth16 Prove ===")
	...
	witnessCircuit, err := NewBkSetRotationCircuit(proofData)
	if err != nil {
	...
	fmt.Println("  groth16_public_inputs.hex  - 2 × 32-byte public inputs (0x-prefixed)")
