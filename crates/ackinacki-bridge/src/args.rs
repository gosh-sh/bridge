...
pub struct WithdrawArgs {
    ...
    /// `BridgeWithdrawalAggregatorVerifier.bin` and `.sol` differ from the
    /// ones this build embeds. Correct only when you deployed your own bridge
    /// and regenerated the verifier.
    ///
    /// This is NOT the counterpart of `aggregate-proof --allow-source-drift`.
    /// That one is a bootstrap escape hatch for the very first run of a
    /// verifier whose source is not committed yet — never use once a verifier
    /// is deployed. Against a live self-deploy the regenerated source must
    /// still equal your own `.sol`, and suppressing that comparison hides a
    /// proof the chain will reject.
    ///
