...
fn test_circuit2_mockprover() {
    ...
    // 1. Build chain data. Depth aligned with the layer keygen path
    // (`bridge-prover-lib/src/keys/layer.rs`) via HISTORY_PROOF_WINDOW_SIZE
    // so the produced siblings match the circuit's tree shape.
    let tree_depth: usize = (bridge_prover_lib::poseidon_dense::HISTORY_PROOF_WINDOW_SIZE + 2)
        .next_power_of_two()
