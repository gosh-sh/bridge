...
with leaves L0..L15 (right subtree currently zero-padded per
poseidon_profile_new spec):

| Leaf   | Meaning                                            |
|--------|----------------------------------------------------|
| L0     | Poseidon(layer_hashes_preimage)                    |
| L1     | SHA256(common_section_bytes)                       |
| **L2** | **old_bk_set_poseidon_hash** (32-byte BE Fr)       |
| **L3** | **new_bk_set_poseidon_hash** (32-byte BE Fr)       |
| L4     | tvm_block_repr_hash                                |
| L5     | SHA256(durable_state)                              |
| L6     | SHA256(tx_cnt u64 BE)                              |
| L7     | Poseidon Merkle root of referenced blocks          |
| L8..L15| reserved (currently zero — right subtree padding)  |

Inner hashes (only the ones on L2/L3's authentication path are named here):
...
**Minimum sibling set to reveal both L2 and L3** = **3 siblings: `h01`,
`h4_7`, and `h8_15`**. Verifier check:

```
h23   = SHA( L2  ‖ L3 )              // L2, L3 are openly provided
h0_3  = SHA( h01 ‖ h23 )             // h01  is sibling
h0_7  = SHA( h0_3 ‖ h4_7 )           // h4_7 is sibling
root  = SHA( h0_7 ‖ h8_15 )          // h8_15 is sibling
require root == block_id_from_Circuit1a/1b
```

The node already exposes the 16 leaves over GraphQL
(`block_merkle_tree_leaves`), so the prover does not have to reconstruct
anything from raw block bytes — it just calls
`BlockIdMerkleTree::from_leaves` and reads `tree.h01`, `tree.h4_7`,
`tree.h8_15`, plus `tree.leaves[2]` and `tree.leaves[3]` (or calls
`tree.siblings_for_l2_l3()` for the 3-element sibling array).
...
### 2.1 `BridgeState` — contract mirror, schema v7
...
pub struct BridgeState {
    ...
    /// schema v7 — seq_no of the last bk-set-update block whose transition
...
### 4.1 New IPC variant (schema v7)

```rust
// ipc.rs (PROOF_REQUEST_SCHEMA_VERSION = 7)
...
pub struct BkUpdateRequest {
    ...
    pub block_id_hex: String,           // raw 32-byte BE root; Fr folded on demand
    ...
    pub merkle_sibling_h01_hex: String,         // sibling of h23 at depth 3
    pub merkle_sibling_h4_7_hex: String,        // sibling of h0_3 at depth 2
    pub merkle_sibling_h8_15_hex: String,       // sibling of h0_7 at depth 1
}
```
...
Backwards compatibility: `ProofRequest` (layer bundle, v7) and
`BkUpdateRequest` (v7) live side-by-side. A discriminator file (or a
...
6. Write `bkupd_{S}.json` with `L2`, `L3`, `h01`, `h4_7`, `h8_15`, and
   the attestation proof. No pubkey list in the IPC.
...
for each new file in proofs/:
    if matches "proof_*.json":
        verify_layer_bundle(...)             // existing — v7 path
    if matches "bkupd_*.json":
        verify_bk_update_bundle(...)         // new — v7 path
```
...
function applyBkSetUpdate(
    ...
    bytes32 siblingH01,                    // depth-3 sibling of h23
    bytes32 siblingH4_7,                   // depth-2 sibling of h0_3
    bytes32 siblingH8_15                   // depth-1 sibling of h0_7
) external {
    ...
    // (2) open depth-4 merkle check — pure sha256, cheap on ETH
...
* Implement schema v7, both daemons, IPC discriminator. On local devnet
...
* Bring up shellnet prover+verifier with the v7 stack.
...
1. `bridge-prover-lib/src/bridge_state.rs`: add `stored_last_bk_set_update_seq_no`
   (commitment-only — NO pubkeys), schema bump to 7, add `apply_bk_set_update(L2, L3, S)`.
...
3. `bridge-prover-lib/src/ipc.rs`: schema constant → 7, add `BkUpdateRequest`
   (no pubkey list, 3 open siblings `h01`/`h4_7`/`h8_15`), new file paths
   (`bkupd_*.json`).
