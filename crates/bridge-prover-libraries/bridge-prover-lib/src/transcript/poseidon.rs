...
//!
//! This is the prover-lib-side counterpart to `snark-verifier-sdk`'s
...
//! produced by any `bridge_prover_lib::…_with_transcript(TranscriptKind::Poseidon, …)`
//! call (which builds against the gosh halo2-lib fork) be fed without
//! modification into an `AggregationCircuit` living in
//! `crates/bridge-evm-aggregator/` (which builds against
...
//! non-optional dependency. This crate pins `halo2-base` to the gosh fork;
//! the two cannot coexist in one cargo unit. So we re-implement the
...
//!   and rejects anything else. Use [`crate::prover::generate_primary_proof`]
//!   / [`crate::prover::generate_fallback_proof`] (default Blake2b path).
