// Phase 3.2 — gnark Groth16 wrapper for Circuit 1A (Primary attestation).
//
// Mirrors `gnark-wrappers/circuit-1b/main.go` 1:1: same 4 public inputs,
// identical CLI surface, identical output artefact names. The only
// differences are the circuit type (PrimaryVerifierCircuit) and on-disk
// paths.
//
// Two subcommands:
//
...
//	    copied into contracts/ethereum/src/PrimaryGroth16VerifierGenerated.sol).
//
//	prove [proof.json]
...
//	go run . setup ../../proofs/primary/halo2_proof.json
//	go run . prove ../../proofs/primary/halo2_proof.json
//	cp Groth16Verifier.sol \
//	   ../../../../contracts/ethereum/src/PrimaryGroth16VerifierGenerated.sol
...
func printUsage() {
	fmt.Println("Usage: gnark-wrapper-1a <command> [proof.json]")
	fmt.Println()
...
func runSetup(proofFile string) {
	fmt.Println("=== Circuit 1A (Primary) Groth16 Setup ===")
	...
	circuit, err := NewPrimaryVerifierCircuit(proofData)
	if err != nil {
...
func runProve(proofFile string) {
	fmt.Println("=== Circuit 1A (Primary) Groth16 Prove ===")
	...
	witnessCircuit, err := NewPrimaryVerifierCircuit(proofData)
	if err != nil {
