// Phase 3.3 — gnark Groth16 wrapper for Circuit 2 (Layer Hashes Movement).
//
// Mirrors `gnark-wrappers/circuit-1a/main.go` 1:1 but with 14 public inputs
// instead of 4 (block_id + bk_set_poseidon + num_layers + 10 layer hashes +
// prev_max_level_layer_hash; see circuit.go for the full layout).
//
// Two subcommands:
//
//	setup [proof.json]
...
//	    verifier; copy into
//	    contracts/ethereum/src/LayerHashesGroth16VerifierGenerated.sol).
//
//	prove [proof.json]
...
// Typical pipeline:
//
//	cargo run --bin export-layer-hashes-proof --release -- --num-layers 1 --num-chain-steps 1
//	cd crates/bridge-prover-orchestrator/gnark-wrappers/circuit-2
//	go run . setup ../../proofs/layer-hashes/halo2_proof.json
//	go run . prove ../../proofs/layer-hashes/halo2_proof.json
...
func printUsage() {
	fmt.Println("Usage: gnark-wrapper-2 <command> [proof.json]")
	fmt.Println()
...
func runSetup(proofFile string) {
	fmt.Println("=== Circuit 2 (Layer Hashes Movement) Groth16 Setup ===")
	...
	circuit, err := NewLayerHashesVerifierCircuit(proofData)
	if err != nil {
...
func runProve(proofFile string) {
	fmt.Println("=== Circuit 2 (Layer Hashes Movement) Groth16 Prove ===")
	...
	witnessCircuit, err := NewLayerHashesVerifierCircuit(proofData)
	if err != nil {
	...
	fmt.Println("  groth16_public_inputs.hex  - 14 × 32-byte public inputs (0x-prefixed)")
