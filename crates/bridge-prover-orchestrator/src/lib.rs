...
//!
//! This crate wraps the partner's halo2 circuits with key/prover/verifier
//! helpers that the orchestrator and the relayer (planned) will consume.
//!
//! Phase 1.A scope: **Circuit 1B (Fallback attestation verifier)** only.
//! Phases 1.B and 1.C will add Circuit 2 (Layer hashes) and Circuit 3 (BK set
//! update).
//!
...
pub use halo2_tvm_bundle::{
    decode_instances, encode_instances, Halo2TvmBundle, TranscriptKind, BUNDLE_MAGIC,
    BUNDLE_VERSION,
};
pub use keys::FallbackKeyManager;
pub use layer_hashes_keys::{
    LayerHashesKeyManager, LayerHashesReferenceWitness, LAYER_HASHES_K, LAYER_HASHES_LOOKUP_BITS,
