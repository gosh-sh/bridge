...
| `WithdrawIdentityMismatch()` | `pub.dappFr`/`pub.accFr` don't match the bridge's stored withdrawal identity | Reverts before crypto (`AckiNackiBridge.sol:1303`). Prover was built against a stale identity — regenerate the witness after the bridge (or its stored identity) is redeployed. See [`WITHDRAW_ACC_FR` derivation](#reference-values-chain-invariant-on-shellnet). |
