...
/// rather than a bad deposit, says which call fixes it. All three fail-closed
/// settings must be re-set by hand after every deploy or code upgrade (they are
/// not threaded through `onCodeUpgrade`), so without this the first deposit after
/// a redeploy reads as a broken bridge.
fn describe_exit_code(code: i32) -> Option<&'static str> {
    match code {
        EXIT_INVALID_ZKPROOF => Some(
            "ERR_INVALID_ZKPROOF — proof rejected by the opcode; check the deployed \
             VK_BLOB is this prover's VkBlob",
        ),
        EXIT_UNKNOWN_SOURCE => Some(
            "ERR_UNKNOWN_SOURCE — source chain/bridge not allowlisted; owner must \
             call setExpectedBridge(chainId, bridgeFr)",
        ),
        EXIT_WRONG_DAPP => Some(
            "ERR_WRONG_DAPP — proof-bound dappId is not this deployment's; owner \
             must call setExpectedAnDappId(dappId)",
        ),
        EXIT_UNKNOWN_BLOCK => Some(
            "ERR_UNKNOWN_BLOCK — the deposit's block is not in the canonical-chain \
             anchor set; confirm canonicality with `scripts/deposit_anchor_params.py \
             --verify`, then owner must call setAcceptedBlockHash(chainId, blockHash, true)",
        ),
        _ => None,
    }
}
...
mod tests {
    ...
    /// The three fail-closed config reverts must name the setter that fixes them
    /// — an operator seeing a bare `exit_code=224` after a redeploy has no way to
