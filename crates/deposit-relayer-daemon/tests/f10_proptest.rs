...
proptest! {
    ...
    #[test]
    fn backoff_next_is_monotone_until_cap(
        initial_ms in 1u64..1000,
        max_ms in 1000u64..60_000,
        multiplier in 2u32..8,
        steps in 1usize..20,
    ) {
        ...
        for _ in 0..steps {
            let next = cfg.next_backoff(current);
            prop_assert!(next >= current || next == cfg.max);
            prop_assert!(next <= cfg.max);
            current = next;
        }
    }

    #[test]
    fn backoff_multiplier_zero_yields_zero(
        ...
        prop_assert_eq!(cfg.next_backoff(cfg.initial), Duration::ZERO);
    }
}
