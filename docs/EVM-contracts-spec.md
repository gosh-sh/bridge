...
| `ShplonkVkDigestPinDerivation.t.sol` (4) | Pins the four `*_VK_DIGEST` constants in `ShplonkDeployLib` to word `12 + N` of the corresponding `_calldata.bin`, so a re-export of the aggregator that changes the digest fails CI unless the constant is regenerated too; a companion assertion pins every constant strictly below `r` so a mix-up with a raw 32-byte hash cannot land unnoticed. |
