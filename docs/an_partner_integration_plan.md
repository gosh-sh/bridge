...
| 4 — Contract rebuild | Independent of partner | 🟢 **In progress** (4.1 done 2026-05-10, additive). `AckiNackiBridge.sol` extended with `verifyBlock(finType, attestationProof, lhProof, …)` consuming 1A/1B + 2 verifiers, cross-circuit + monotonic + chain-anchor invariants, 17 new Foundry tests (real bound proofs end-to-end + Fallback-mock + 14 negative paths). 4.2 (delete legacy `LayerHashBridge.sol` + migrate its 27+4+14 tests) deferred until Phase 5/6 relayer-driven path lands. |
...
Phases 3.1, 3.2, 3.3 are now live (Circuits 1B + 1A + 2); only Phase 3.4 (BK-set update) remains and it's gated on Phase 1.C. **Phase 4.1 is done as of 2026-05-10** (additive `verifyBlock` on `AckiNackiBridge.sol` with cross-circuit consistency + chain-anchor invariants; 178/178 Foundry tests green including 17 new). **Phase 5.1 is done as of 2026-05-10** (relayer skeleton — `crates/bridge-relayer-daemon/` with `Relayer::tick()`/`run_loop()` + `BlockSource`/`BridgeClient` traits + production `EthBridgeClient` (abigen) and in-memory mocks for fast unit testing + atomic `state.json` persistence + CLI binary; 13 Rust unit tests + 6 Foundry tests in `AckiNackiBridgeRelayerLoop.t.sol` driving 10 sequential blocks through `verifyBlock` via mock verifiers, all green; 184/184 Foundry tests across 19 suites). The natural next moves are: (a) **Phase 5.2** (`LiveBlockSource` impl backed by partner's `gql_client` + `boc_parser` + our `bridge-prover-orchestrator` for halo2+gnark wrapping inside the relayer); blocked on Q1 + Q2. (b) **Phase 5.3** (10-block shellnet end-to-end against Anvil — the §5 acceptance criterion). (c) **Phase 4.2** (delete legacy `LayerHashBridge.sol` + migrate its 45 tests onto `AckiNackiBridge.verifyBlock` once a real-data 2-block chain-anchor advancement scenario exists from the relayer). (d) **Phase 1.C** (Circuit 3 wiring + Phase 3.4 Solidity verifier + extending `verifyBlock` with an optional `bkSetUpdateProof` argument), once Alina signals.
