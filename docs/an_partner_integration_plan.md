...
- 3.4 Generate **development-grade** per-circuit Groth16 keys via `groth16.Setup(ccs)` (single-party, locally on dev box). Sufficient for dev/testnet; **mainnet deployment is explicitly gated on Phase 8 MPC ceremony** (see below). Each `Setup` call must be reproducible: pin gnark version + circuit R1CS hash + Phase 1 SRS source in `docs/gnark_ceremony.md`. The VKs emitted here become the artefacts that the Phase 8 ceremony replaces, not augments.
...
**`v2.0.0` (mainnet)** requires Phase 8 (trusted setup ceremony) on top of everything in 7.9.
...
- 7.9 🟡 Release tag `v2.0.0-rc1` — release-manager step. Gates listed in `docs/audit_trail_v2.md` §6 (Phase 5.2/5.3/1.C, BLS-1/FORK-2 mitigation, CI green). `v2.0.0-rc*` ships with dev-grade Groth16 keys (single-party `Setup`); the **mainnet** `v2.0.0` tag additionally requires **Phase 8** complete (trusted setup ceremony + verifier rotation).
...
### Phase 8 — Trusted Setup Ceremony (Pre-Mainnet, **REQUIRED**)

**Status**: 🔴 Not started. **Mainnet-blocking**. Testnet/RC tags are *not* blocked (single-party `Setup` is acceptable under "no real funds, no real liveness guarantee" assumption); the gate is **`v2.0.0` (mainnet) ⇒ ceremony complete and on-chain verifier VKs replaced**.

**Why this is a separate phase**: see R14 above. The current `groth16.Setup(ccs)` calls in all four wrappers (`deposit-prover/gnark-wrapper`, `crates/bridge-prover-orchestrator/gnark-wrappers/circuit-{1a,1b,2}/`, and the future `circuit-3/` once Phase 1.C lands) generate Phase 2 keys with toxic waste held in a single process. Anyone with that waste can forge wrapper proofs and bypass Halo2 verification entirely. This is *not* an academic concern: the wrapper VK is what `*Groth16VerifierGenerated.sol` enshrines on-chain, and the on-chain verifier never sees the Halo2 SHPLONK proof underneath — only the wrapper's Groth16 proof.
...
| R14 | gnark Groth16 trusted setup per circuit (4 ceremonies) — **single-party Setup is currently a bridge-wide soundness gap** | Confirmed open as of 2026-05-17 | **Critical for mainnet** | All four wrappers (`deposit-prover/gnark-wrapper`, `circuit-1a/`, `circuit-1b/`, `circuit-2/` under `crates/bridge-prover-orchestrator/gnark-wrappers/`) currently call `groth16.Setup(ccs)` in-process. That means the Phase 2 toxic waste (α, β, γ, δ) was held in a single process on a single developer machine; if it leaked, the holder can forge any wrapper proof and **fully bypass Halo2 verification on Ethereum** (the contract only checks the Groth16 wrapper, not the underlying Halo2 SHPLONK proof). Phase 1 (Powers of Tau) is the universal trusted-setup component and is reusable from a public ceremony — currently we're also generating it implicitly inside `groth16.Setup`, ignoring the available public artefacts (Hermez `pot28_final.ptau`, Perpetual PoT, Aztec Ignition). Mitigation = **Phase 8** (new): (a) switch to a public Phase 1 SRS, (b) run an MPC Phase 2 ceremony per circuit with N ≥ 5 independent contributors, (c) re-deploy the four `*Groth16VerifierGenerated.sol` contracts with ceremony VK hashes, (d) publish per-circuit contribution audit trail. Same trust model that today's deposit prover and the v1 layer-hash prover have inherited from single-party `Setup` — i.e., the bridge currently has the same gap they had; we're being explicit about closing it before mainnet rather than carrying it forward. |
...
| 8 — Trusted setup ceremony (pre-mainnet, calendar-bound) | ~5 engineering days + 2–3 weeks calendar |
| **Total (engineering)** | **≈ 35 working days = 7 weeks engineer-time** |
| **Total (calendar, mainnet-ready)** | **≈ 9–10 weeks** (Phase 8 cannot be parallelised with itself across contributors but can overlap Phase 7) |

This assumes Phase 0 returns "all clear" within 2 days. Each "Medium" risk that materialises adds 2–5 days; an R1 or R2 hit would push the timeline closer to 8–9 weeks. Phase 8 is **calendar-bound** rather than effort-bound — the engineering work is ~5 days (Phase 1 SRS adoption + Phase 2 MPC scripts + verifier rotation + CI guard + audit-trail doc) but the MPC contribution chain takes 2–3 weeks regardless of how many engineers we throw at it.
...
Q1 + Q2 gate Phase 1.C. Q3 + Q4 affect Phase 5.2 cleanliness. Q5 is CI-hermeticity polish. Phases 5.2 (Primary-only) and 5.3 can otherwise start now.
