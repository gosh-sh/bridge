...
Phases 3.1, 3.2, 3.3 are now live (Circuits 1B + 1A + 2); only Phase 3.4 (BK-set update) remains and it's gated on Phase 1.C. **Phase 4.1 is done as of 2026-05-10** (additive `verifyBlock` on `AckiNackiBridge.sol` with cross-circuit consistency + chain-anchor invariants; 178/178 Foundry tests green including 17 new). The natural next moves are: (a) **Phase 5** (relayer skeleton — fetch envelope/attestation/chain proofs, drive 1A-or-1B + 2 against the orchestrator, submit to `verifyBlock`); Phase 5 can pick up partner's `latest_an_to_eth_bridge_test_lightweight` branch for live AN node testing. (b) **Phase 4.2** (delete legacy `LayerHashBridge.sol` + migrate its 45 tests onto `AckiNackiBridge.verifyBlock` once a real-data 2-block chain-anchor advancement scenario exists from the relayer). (c) **Phase 1.C** (Circuit 3 wiring + Phase 3.4 Solidity verifier + extending `verifyBlock` with an optional `bkSetUpdateProof` argument), once Alina signals.
