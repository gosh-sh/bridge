...
**Companion docs**:
- `docs/architecture/four_circuit_architecture.md` — architecture detail.
- `docs/operations/bridge_verification.md` — invariant labels (DEP-#, LH-#, BK-#, OR-#, AC-#, FORK-#, CC-#, ZK-#).
- `docs/integration/an_partner_integration_plan.md` — phase-by-phase roadmap.
...
**Net code-surface impact (Phase 4.2)**: -49 Foundry tests retired with legacy suites; current suite: **174 tests / 21 suites** (2026-07).

**Phase 4.3 demolition (2026-05-17)** — legacy ETH-side deposit-verifier chain retired:

| Surface retired | LoC retired | Replacement |
|---|---|---|
| `AckiNackiBridge.withdraw(...)` + `processedDeposits` mapping + `isDepositProcessed(...)` view + `Withdrawal` event + 4 errors + `_verifier` ctor arg + `verifier` storage slot | ~60 lines inside `AckiNackiBridge.sol` | None on the ETH side. The ETH→AN deposit proof is now consumed on the AN side via `ZKHALO2VERIFYWITHVK` (K-13). |
...
**Net code-surface impact after Phase 4.3+**: **174 Foundry tests / 21 suites** (2026-07). `forge build`, `forge test`, and `cargo check --workspace --all-targets` expected clean.
...
before mainnet launch. Tracked as a launch blocker in `docs/integration/an_partner_integration_plan.md`
risk register R12.
...
|---|---|
| Detailed Halo2 chip-level audit | `docs/audit/layer_hashes_circuit_audit.md` (legacy, but the chips are unchanged in v2) |
| Manual hands-on verification protocol | `docs/operations/manual_verification_runbook.md` |
| Property-driven invariant reference | `docs/operations/bridge_verification.md` |
| Per-circuit native-Halo2 proof verification flow | `docs/operations/verifying_an_proof.md` |
| AAVE V3 yield bolt-on | `docs/operations/aave/aave_integration.md` |
| Phase-by-phase implementation plan | `docs/integration/an_partner_integration_plan.md` |
...
- [x] Current Foundry suite: **174 tests / 21 suites** at HEAD (2026-07).
