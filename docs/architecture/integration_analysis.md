...
> overview lives in `docs/architecture/four_circuit_architecture.md`.
>
> **Updated 2026-07.** Deposits are USDC; `ZKHALO2VERIFYWITHVK` deployed on shellnet. Production AN→ETH verifiers use R15 SHPLONK aggregators (not gnark Groth16). See `docs/README.md` for current architecture index.
...
Main bridge contract with three permissionless entry points and optional AAVE yield:

- **`deposit(amount, anWorkchain, anAccount)`** — accepts USDC (max 100 USDC, 6 decimals) via `transferFrom`, increments `depositCounter`, emits `Deposit(depositId, sender, amount, anWorkchain, anAccount, timestamp)`.
- **`verifyBlock(...)`** — verifies Circuits 1A/1B + 2 via R15 SHPLONK aggregator adapters; enforces cross-circuit binding and monotonicity.
- **`withdrawByProof(proof, pub)`** — Circuit 4 payout: verifies withdrawal proof, checks anchor, pays USDC to EVM recipient.
...
- AAVE: immutable `aavePool` / `wethGateway` / `aWETH` (USDC/aUSDC in production wiring); `aaveEnabled`, `suppliedPrincipal`, `liquidReserveBps`.
...
See `docs/operations/aave/aave_integration.md` for the AAVE design, invariants, and verification protocol. See `docs/architecture/four_circuit_architecture.md` for the cross-circuit binding semantics.
...
```
Ethereum deposit event (deposit() on AckiNackiBridge — USDC)
    → deposit-prover (Rust/axiom-eth): Halo2 circuit
        ...
        - Contract address + AN recipient binding
        - Block hash binding
        → 11 public inputs: [depositId, sender, amount, contractAddress,
                            dappIdHigh, dappIdLow, anAccountHigh, anAccountLow,
                            blockHashHigh, blockHashLow, promiseCommit]

    → AN-side native verification via ZKHALO2VERIFYWITHVK (deployed)
        - USDCBridge.finalizeDeposit(proof, publicInputs)
        - ECC minted to proven AN recipient; depositId nullifier prevents replay
```

The keccak coprocessor pattern (see `docs/architecture/diagrams/keccak_coprocessor_flowchart.mmd`) delegates expensive keccak256 computations to a separate circuit via Poseidon-based promise commitments.

### 1.4 Integration Surfaces

**`crates/acki-nacki-interface`**: Live REST client (`BkSetClient`, `BkSetTracker`) + TVM submitter traits for AN interaction.

**`crates/deposit-relayer-daemon`**: EVM→AN relayer — listens for deposits, runs `deposit-prover`, submits `finalizeDeposit`.

**`crates/bridge-relayer-daemon`**: AN→ETH relayer — `verifyBlock` + `withdrawByProof` via `daemon-bridge`.

**`crates/an-bridge-prover/`**: Live AN→ETH prover sub-workspace (daemons, Circuit 4).

**`crates/bridge-prover-orchestrator/`**: Fixture export + R15 aggregator glue (not the live prover).

**`crates/bridge-prover-orchestrator/gnark-wrappers/`**: Legacy gnark wrappers retained for Foundry test coverage only. Production uses R15 SHPLONK `.bin` verifiers.
...
See `docs/architecture/four_circuit_architecture.md` §1 for the canonical table. Quick summary:
...
**Blake2b** Fiat-Shamir transcript with `Challenge255` for challenge squeezing. This is the transcript the Ethereum-side verifier must reproduce when verifying Halo2 proofs directly, and the one the AN→ETH gnark wrappers must understand when wrapping (the ETH→AN deposit-prover side uses a Keccak transcript instead, since its proof is consumed natively on the AN side via `ZKHALO2VERIFYWITHVK`).
...
| AAVE V3 yield bolt-on | Complete (20 tests, see docs/operations/aave/aave_integration.md) |
| Block-hash oracle (Axiom V2) | Complete (currently unused by the public surface; reserved for the future burn-proof flow) |
| Deposit-prover Halo2 circuit (ETH→AN) | Complete; consumed natively on the AN side via `ZKHALO2VERIFYWITHVK` (in development) — no gnark wrapper |
...
See `docs/architecture/four_circuit_architecture.md` §8 for the canonical list. Five legacy assumptions
...
- **Added**: trust in (a) the new `ZKHALO2VERIFYWITHVK` TVM opcode implementation in `tvm-sdk`
  and its CI/audit story, and (b) the AN-side `TokenBridge` contract that will call it. Both
  are tracked in `docs/integration/an_partner_integration_plan.md` Decision Log 2026-05-17 and Phase 8.
