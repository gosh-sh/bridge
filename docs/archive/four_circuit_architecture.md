...
**Supersedes**: the legacy single-circuit narrative in `docs/integration_analysis.md` §3 and the layer-hash sections of `docs/integration_plan.md` (M7–M9). Those docs are now historical record only.
**Read first**: `AGENTS.md` for a one-page tour; this doc for the architectural details; `docs/an_partner_integration_plan.md` for the live phase-by-phase roadmap; `docs/bridge_verification.md` for invariant labels.
...
All public inputs are BN254 `Fr` field elements (`uint256` mod the curve order, gnark MarshalSolidity layout). All proofs are exactly 256 bytes (Groth16 wrapped from Halo2 SHPLONK by the per-circuit gnark wrappers under `crates/bridge-snark-utils/gnark-wrappers/`).
...
The `blockId` and `bkSetCommitment` parameters are passed **once** but flow into both verifier calls. The two gnark Groth16 verifiers each have their own VK, so the `blockId` that the bridge supplies must match the one each prover committed. There is no way to make Circuit 1A's proof accept block X's `blockId` and Circuit 2's proof accept block Y's `blockId` while only paying one shared argument — one of the verifications will fail.
...
|---|---|
| **CC-1** Same block | `blockId` argument flows into both verifier calls — any mismatch surfaces as a gnark `false` |
| **CC-2** Same committee | `bkSetCommitment` argument flows into both verifier calls — same enforcement as CC-1 |
...
[Acki Nacki node]
   ...
   ▼
[bridge-relayer-daemon]                 ← Rust crate, Phase 5.1 done (mock sources only)
   ...
   │  4. (Phase 5.2) Recompute transition_hashes locally (Plan B for Q2)
   ▼
[bridge-snark-utils]            ← Rust crate, Phase 4 done
   │  Per circuit: Halo2 SHPLONK prove → gnark Groth16 wrap (256-byte proof)
   ▼
[Ethereum: AckiNackiBridge.verifyBlock]
   │  1. Cheap shape & anchor checks (revert before crypto on a mismatch)
   │  2. primaryVerifier.verifyPrimaryAttestation(...)  // or fallbackVerifier
   │  3. layerHashesVerifier.verifyLayerHashesMovement(...)
   │  4. Effects: advance the state machine, emit BlockVerified
```
...
| Stage | Cost | Notes |
|---|---|---|
| Halo2 SHPLONK prove (per circuit) | ~3-6 min | K=19, BN254; one PK per circuit (~2 GB each) |
| gnark Groth16 wrap | ~5-15 s | done by `crates/bridge-snark-utils/gnark-wrappers/` |
| `verifyBlock` on Ethereum | ~440k gas | two pairing checks (one per Groth16 proof) + storage updates |

---

## 7. Test Surface

The `forge test` suite (135 tests across 15 suites, all green at HEAD) covers `verifyBlock` from two angles:

| Suite | Count | What it covers |
|---|---|---|
| `AckiNackiBridgeVerifyBlockTest` (`test/AckiNackiBridgeVerifyBlock.t.sol`) | 17 | Single-block surface. **One real bound proof set** (Circuit 1A + Circuit 2 generated by `cargo run -p bridge-snark-utils --bin export-bound-block-proofs`) drives the happy path; mocks drive everything else. Negative cases: wrong `blockId`, wrong `bkSetCommitment`, wrong anchor, wrong shape, wrong finalization type, wrong proof bytes, mismatched `prevMaxLevelLayerHash`, tail-non-zero, `numLayers` out of range, both verifiers rejecting, only one rejecting. |
| `AckiNackiBridgeRelayerLoopTest` (`test/AckiNackiBridgeRelayerLoop.t.sol`) | 6 | Multi-block sequencing. Mock verifiers + 10 sequential blocks. Asserts state advances, restart-recovery is consistent, `replaySameSeqNo` reverts with `BlockSeqNoNotMonotonic`, `anchorMismatch` reverts with `PrevAnchorMismatch`, `attestationReject` reverts with `AttestationProofRejected`. |
| `PrimaryVerifierTest`, `FallbackVerifierTest`, `LayerHashesMovementVerifierTest` | 12 total | Per-adapter sanity: 256-byte length check, decoder layout, try/catch normalisation of gnark reverts to `false`, constructor zero-address rejection. |

Plus the unchanged deposit/AAVE/oracle/fuzz suites (`AckiNackiBridgeV2Test`, `AckiNackiBridgeAaveTest`, `AxiomBlockHeaderOracleTest`, `Blake2b/KeccakHalo2VerifierTest`, etc.).

The Rust side has unit tests in `crates/bridge-snark-utils/tests/` and `crates/bridge-relayer-daemon/src/{relayer,bridge,source,state}.rs` (`#[cfg(test)]` modules). Run with `cargo check --workspace && cargo test -p bridge-snark-utils && cargo test -p bridge-relayer-daemon`.
...
| `gosh-halo2-crypto-lib` correctness | Audited (`docs/layer_hashes_circuit_audit.md`); two open mediums (BLS-1 / FORK-2: G2 subgroup gap) carry over to v2 because Circuit 1A/1B reuse the same BLS gadget. Severity unchanged from the legacy assessment. |
...
- `contracts/ethereum/src/{Primary,Fallback,LayerHashes}Groth16VerifierGenerated.sol` — gnark output (regenerate from `crates/bridge-snark-utils/gnark-wrappers/{circuit-1a,circuit-1b,circuit-2}/main.go`).
...
- `crates/bridge-snark-utils/` — Rust prover orchestrator.
...
| `IBridgeEventGroth16Verifier.sol` | Interface for the gnark-generated 103-input verifier; the production `BridgeEventGroth16VerifierGenerated.sol` will come from `crates/bridge-snark-utils/gnark-wrappers/circuit-4/` once Phase B circuit changes land. |
...
| **Bound test data** | A synthetic block scenario where Circuit 1A/1B and Circuit 2 share `block_id` and `bk_set_poseidon` — generated by `crates/bridge-snark-utils/src/bound_test_data.rs` and exported by `bin/export_bound_block_proofs.rs`. |
| **gnark wrapper** | Per-circuit Go module under `crates/bridge-snark-utils/gnark-wrappers/` that wraps a Halo2 SHPLONK proof into a 256-byte Groth16 proof verifiable by an EVM pairing check. |
