...
- **DEP-N-4**: half contract, half trust assumption. The contract half is `require(_acceptedBlockHash[f.chainId][_parseBlockHash(publicInputs)], ERR_UNKNOWN_BLOCK)` in `USDCBridge.finalizeDeposit` (`acki-nacki` `7992ce26`), mirroring `AckiNackiBridge._knownAnchors` in the AN→ETH direction. No circuit can supply the other half — "this header is canonical" is a statement about Ethereum consensus, not about the witness (BC-D01) — so the anchor set is populated from outside the proof, today by the owner key via `setAcceptedBlockHash`. **That key is therefore trusted for source-chain canonicality**: it can mint by admitting a hash from a chain that never existed. M-of-N attesters are the planned replacement; an ETH light client is the long-run target for L1. `scripts/deposit_anchor_params.py --verify` enforces the independence + confirmation-depth obligations before printing the setter arguments. Full analysis: `docs/reviews/deposit_circuit_audit_2026-08-03.md` §1.
