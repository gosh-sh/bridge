...
All public inputs are BN254 `Fr` field elements (`uint256` mod the curve order, gnark MarshalSolidity layout). All proofs are exactly 256 bytes (Groth16 wrapped from Halo2 SHPLONK by the per-circuit gnark wrappers under `crates/bridge-prover-orchestrator/gnark-wrappers/`).
...
[bridge-relayer-daemon]                 ← Rust crate, Phase 5.1 done (mock sources only)
   ...
   ▼
[bridge-prover-orchestrator]            ← Rust crate, Phase 4 done
...
| gnark Groth16 wrap | ~5-15 s | done by `crates/bridge-prover-orchestrator/gnark-wrappers/` |
...
The Rust side has unit tests in `crates/bridge-prover-orchestrator/tests/` and `crates/bridge-relayer-daemon/src/{relayer,bridge,source,state}.rs` (`#[cfg(test)]` modules). Run with `cargo check --workspace && cargo test -p bridge-prover-orchestrator && cargo test -p bridge-relayer-daemon`.
