...
> were retired.
>
> **v2.4 update (2026-07).** Documentation refactor + current test surface:
> - Foundry: **174 tests across 21 suites** (includes `withdrawByProof`, production SHPLONK, pause, BK-update).
> - Deposits are **USDC** (`transferFrom`), not native ETH. AN opcode: **`ZKHALO2VERIFYWITHVK`** (deployed).
> - Circuit 4: **`withdrawByProof`** (10 PI), not Phase A `verifyEvent` (103 PI).
>
...
> - `crates/bridge-snark-utils/proofs/bound/` now contains a fallback
...
This is the procedural twin of `docs/operations/bridge_verification.md` (which states *what* must hold). Here you actually run things and tick boxes.
...
| 3 | C — Automated Tests | 1 min | 174/174 green (21 suites) |
...
crates/                             ← Rust workspace
  bridge-snark-utils/       ← Halo2 prover orchestrator + per-circuit gnark wrappers (1A/1B/2)
...
docs/README.md                           ← One-page project context
```

The legacy `layer-hashes-prover/` and `bk-set-rotation-prover/` directories were retired in
Phase 4.2 (2026-05-10). Their successors live under `crates/bridge-snark-utils/`.

### A.2 Read the one-page summary

```bash
sed -n '1,60p' docs/README.md
```
...
```
Ran 21 test suites in ...: 174 tests passed, 0 failed, 0 skipped
```
...
Per-suite expectation (2026-07, 21 suites / 174 tests):

| Suite | Tests |
|---|---|
| `AckiNackiBridgeAaveTest` | 21 |
| `AckiNackiBridgeVerifyBlockTest` | 17 |
| `FuzzAckiNackiBridgeVerifyBlockTest` | 7 |
| `AckiNackiBridgeRelayerLoopTest` | 6 |
| `AckiNackiBridgeWithdrawByProofTest` | 27 |
| `AckiNackiBridgeWithdrawByProofOrder2Test` | 1 |
| `AckiNackiBridgeProductionVerifyBlockTest` | 4 |
| `AckiNackiBridgeProductionWithdrawByProofTest` | 3 |
| `AckiNackiBridgePauseTest` | 12 |
| `AckiNackiBridgeLayerAnchorTest` | 4 |
| `AckiNackiBridgeApplyBkSetUpdateTest` | 3 |
| `AxiomBlockHeaderOracleTest` | 16 |
| `Blake2bHalo2VerifierTest` | 8 |
| `Halo2PoseidonVerifierTest` | 7 |
| `FuzzVerifiersTest` | 9 |
| `PrimaryVerifierTest` | 8 |
| `LayerHashesMovementVerifierTest` | 10 |
| `ShplonkAggregatorForgeryTest` | 3 |
| `ShplonkDeployLibTest` | 2 |
| `ShplonkSpikeOnChainTest` | 2 |
| `AckiNackiBridgeAaveForkTest` (opt-in, needs `FORK_URL`) | 4 |
| **Total** | **174** |
...
- `AckiNackiBridge.deposit` ≈ 90k gas (USDC `transferFrom`, no AAVE).
- `AckiNackiBridge.verifyBlock` ≈ 700k gas (R15 SHPLONK aggregators + storage).
- (Legacy `Groth16DepositVerifier.verify` ≈ 287k gas was retired in Phase 4.3; deposit-proof verification now happens natively on the AN side via `ZKHALO2VERIFYWITHVK`.)

---

## 4. Phase D — Single-Block Bound Real-Proof (≈ 1 min)

This is the **smoking gun** test for v2: a tuple of real bound Groth16 proofs (Circuit 1A + Circuit 2) generated by `crates/bridge-snark-utils`, verified on-chain through `AckiNackiBridge.verifyBlock`.
...
ls -la crates/bridge-snark-utils/proofs/bound/{primary,fallback,layer-hashes}/
cat crates/bridge-snark-utils/proofs/bound/bound_scenario.json | jq .
```

✅ Expected: under `crates/bridge-snark-utils/proofs/bound/`:
...
> **Retired in Phase 4.3 (2026-05-17)**: the legacy `withdraw(recipient, amount, depositId, blockNumber, proof)` cast probes (former E.4–E.8), the `isDepositProcessed(...)` view, `TestDepositVerifier` deployment, and the `verifier()` getter are no longer applicable — the legacy refund-style withdrawal mechanism was removed. The equivalent acceptance / rejection scenarios moved to the AN-side `TokenBridge.finalizeDeposit(...)` path, exercised via `tvm-sdk` tests once `ZKHALO2VERIFYWITHVK` ships (see `docs/operations/verifying_eth_proof_on_an.md` §2).
...
WIRE_VERIFY_BLOCK=true \
  GENESIS_BK_SET_COMMITMENT=$(cat ../../crates/bridge-snark-utils/proofs/bound/bound_scenario.json \
                              | jq -r '.bk_set_poseidon_decimal') \
  GENESIS_PREV_MAX_LEVEL_LAYER_HASH=$(cat ../../crates/bridge-snark-utils/proofs/bound/bound_scenario.json \
...
- BK-1..BK-5 invariants (see `docs/operations/bridge_verification.md` §6.2 for the target list).
...
The full design is in `docs/operations/aave/aave_integration.md`. Here we run the runtime checks.
...
The attacks are grouped by surface. Numbers in **bold** are the property labels from `docs/operations/bridge_verification.md` (DEP-#, LH-#, BK-#, OR-#, AC-#, FORK-#, **CC-#**, ZK-#).
...
**[Retired — Phase 4.3]** The `processedDeposits[depositId]` nullifier and the entire withdrawal-side double-spend protection moved to AN-side `TokenBridge.finalizeDeposit(...)`. The Halo2 deposit-prover circuit is unchanged; only the consumer side is gone on Ethereum. See `docs/operations/verifying_eth_proof_on_an.md` §2 V5 for the AN-side nullifier responsibility.

#### J.2 Reuse a proof for a different `depositId`

**[Retired — Phase 4.3]** With the legacy ETH-side `Groth16DepositVerifier` removed, this attack now applies to the AN-side native verifier (`ZKHALO2VERIFYWITHVK`). The cryptographic binding is the same — `depositId` is a public input of the Halo2 circuit, and the verifier rejects any proof whose witnessed `depositId` doesn't match. Coverage moves to `tvm-sdk` opcode tests + AN-side `TokenBridge` tests once those land.

#### J.3 Submit a proof for a **different bridge** to this bridge

**[Reframed — Phase 4.3]** Same threat model, now AN-side. `TokenBridge.finalizeDeposit` will enforce `require(publicInputs[3] == ETH_BRIDGE_ADDRESS_FR, "wrong bridge contract");`. The Halo2 circuit's `contractAddress` public input is the binding hook. See `docs/operations/verifying_eth_proof_on_an.md` DEP-N-2.
...
```bash
# Programmatic — see crates/bridge-snark-utils/tests/cross_block_binding.rs
...
|---|---|---|---|
| J.1 | Replay withdraw | **[Retired Phase 4.3]** AN-side nullifier in `TokenBridge.finalizeDeposit` | `tvm-sdk` opcode tests (once `ZKHALO2VERIFYWITHVK` lands) |
...
- **`ZKHALO2VERIFYWITHVK` opcode soundness (new in Phase 4.3)**: the AN-side deposit verification depends on the correctness of the new TVM opcode in `tvm-sdk`. The opcode is in development; CI coverage + a partner-side review are required before mainnet. Tracked under Decision Log 2026-05-17 + Phase 8.
...
These are documented in `docs/architecture/audit_trail_v2.md` §2 and `docs/integration/an_partner_integration_plan.md` §5 (Risk Register).
...
- [ ] docs/README.md was read and matches what's actually in the repo.
...
- [ ] Residual risks (51% on L1, >2/3 on AN, G2 subgroup, gnark stub on AN→ETH side, `ZKHALO2VERIFYWITHVK` opcode soundness, trusted setup, no-rotation gap) acknowledged and tracked in `docs/architecture/audit_trail_v2.md` §2 and `docs/integration/an_partner_integration_plan.md` §5.
...
- [ ] After deployment, run all `cast call` checks from `docs/operations/bridge_verification.md` §11 — every immutable matches its expected value.
...
- `docs/operations/bridge_verification.md` for the property statement (DEP-#, LH-#, BK-#, OR-#, AC-#, FORK-#, CC-#, ZK-#).
- `docs/architecture/four_circuit_architecture.md` for the architectural context.
- `docs/architecture/audit_trail_v2.md` for the trust-assumption delta.
- `docs/audit/layer_hashes_circuit_audit.md` for circuit-level details.
