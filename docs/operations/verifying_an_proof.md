...
For the *what-it-proves* and *how-it-was-built* context, see `docs/architecture/four_circuit_architecture.md` and `docs/audit/layer_hashes_circuit_audit.md`. This doc is purely procedural.

> **v1 history**: the legacy single-circuit walkthrough lives at `_archive/docs/legacy/verifying_an_proof_v1.md`. It is no longer canonical; v1 proofs cannot be submitted to the v2 `AckiNackiBridge.verifyBlock` (different VKs, different public-input shape).
...
See `docs/architecture/four_circuit_architecture.md` §4 for the full CC-# table.
...
- `layer_hashes[i]` mismatch for some `i < num_layers` → bincode layout drift in the AN node (audit finding L-1; see `docs/architecture/audit_trail_v2.md` K-11).
...
|---|---|
| Architecture & cross-circuit binding | `docs/architecture/four_circuit_architecture.md` |
| Invariant labels (LH-#, CC-#, etc.) | `docs/operations/bridge_verification.md` |
| Hands-on manual review protocol with attack scenarios | `docs/operations/manual_verification_runbook.md` |
| Mirror flow (Ethereum → AN deposit verification) | `docs/operations/verifying_eth_proof_on_an.md` |
| Phase-by-phase implementation status | `docs/integration/an_partner_integration_plan.md` |
| Trust-assumption delta v1 → v2 | `docs/architecture/audit_trail_v2.md` |
| Legacy v1 walkthrough (single-circuit, 13 PI) | `_archive/docs/legacy/verifying_an_proof_v1.md` |
