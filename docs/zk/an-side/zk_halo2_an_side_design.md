...
**Date**: 2026-05-17.
> **Design memo (historical context).** For the frozen wire ABI, see [zkhalo2verifywithvk_reference.md](zkhalo2verifywithvk_reference.md). Decision Log entries referencing `AGENTS.md` are preserved in [an_partner_integration_plan.md](../integration/an_partner_integration_plan.md).
...
We added a **second opcode** rather than re-purposing `ZKHALO2VERIFY`, for the reasons captured in `docs/integration/an_partner_integration_plan.md` Decision Log 2026-05-17:
...
> Decision Log entry in `docs/integration/an_partner_integration_plan.md` and a bumped
...
`docs/integration/an_partner_integration_plan.md` Decision Log 2026-05-22.
...
Owner: bridge team.
Scope: add `gosh.zkHalo2VerifyWithVK(...)` in the spirit of `gosh.vergrth16WithVK(...)` (see Decision Log 2026-05-17 in [an_partner_integration_plan.md](../integration/an_partner_integration_plan.md)).
...
| H6 | VK fixture lifecycle: which version of `deposit-prover`'s circuit ships with which deployment of `TokenBridge` | Document in `docs/operations/verifying_eth_proof_on_an.md` §4 V4: VK is immutable per `TokenBridge` deployment; circuit upgrades require a contract upgrade. |
...
- `docs/integration/an_partner_integration_plan.md` Decision Log 2026-05-17 — Phase 4.3 rationale.
- `docs/architecture/audit_trail_v2.md` §1 R-8/R-9 — trust delta from Phase 4.3.
- `docs/operations/verifying_eth_proof_on_an.md` — operational verification flow.
