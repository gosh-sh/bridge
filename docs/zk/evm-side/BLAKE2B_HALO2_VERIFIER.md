...
The whole left-hand chain was retired in Phase 4.3. The ETH→AN deposit-event proof is now consumed natively on the AN side via the future `ZKHALO2VERIFYWITHVK` TVM opcode (in development in `tvm-sdk`). The `IBlockHeaderOracle` family is preserved in the tree but currently unused by the public surface; it is the building block for the future burn-proof ETH-side withdrawal flow.
