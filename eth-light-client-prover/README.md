...
| M5 — relayer | **landed** (`crates/eth-light-client-relayer`). Live loop: real committee from `light_client/updates`, `submitRotate` **on** by default (`--no-rotate` opts out), systemd **without** `--dry-run --mock-prove --no-rotate`. Flip: `disableOwnerAnchors` + `disableOwnerRotation`. Ancestry: `submit-ancestry`. E2E: `scripts/ursus/eth_lc_shellnet_e2e.md`. |
| M-audit | **in this PR** — `docs/m_audit_scope.md` (includes tvm-sdk#284 opcode decider) |
| M6 — testnet E2E | **runbook in this PR** — `scripts/ursus/eth_lc_shellnet_e2e.md` (operator on n14 + shellnet) |
